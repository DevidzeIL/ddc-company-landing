...
export function Cases() {
  ...
  const regionLabel = locale === "de" ? "Projektreferenzen" : "Примеры проектов";
  const nextLabel = locale === "de" ? "Nächstes Projekt" : "Следующий проект";
  const dotLabel = (i: number) => locale === "de"
    ? `Projekt ${i + 1} von ${t.homeCases.cases.length}: ${t.homeCases.cases[i].title}`
    : `Проект ${i + 1} из ${t.homeCases.cases.length}: ${t.homeCases.cases[i].title}`;

  return (
    <section id="case" className="py-10 md:py-16 px-6" aria-label={regionLabel}>
      <div className="max-w-[1200px] mx-auto">
        <div className="flex flex-col lg:flex-row gap-6 md:gap-10">
          ...
          <div className="flex-1" aria-live="polite" aria-atomic="true">
            ...
            <div className="mb-6 md:mb-8">
              <p className="font-mono text-xs text-white/40 uppercase tracking-wider" aria-hidden="true">{t.homeCases.before}</p>
              <p className="font-mono text-sm md:text-base text-white/80 mt-2 leading-relaxed">{c.before}</p>
            </div>

            <div className="mb-6 md:mb-10">
              <p className="font-mono text-xs text-white/40 uppercase tracking-wider" aria-hidden="true">{t.homeCases.after}</p>
        ...
        {/* Dots indicator */}
        <div
          className="flex gap-2 mt-6 md:mt-8 justify-center"
          role="group"
          aria-label={locale === "de" ? "Projektauswahl" : "Выбор проекта"}
        >
